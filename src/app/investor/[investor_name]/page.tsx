...
export default function InvestorPage() {
  ...
  const investorType = data?.data?.[0] ? getInvestorTypeName(data.data[0].INVESTOR_TYPE) : "Unknown Type";
  const origin = data?.data?.[0]?.LOCAL_FOREIGN === "L" ? "Local" : "Foreign";

  return (
    <div className="space-y-6">
      <Link href="/" className="inline-flex items-center text-sm font-medium text-blue hover:text-foreground transition mb-2">
      ...
      <div className="bg-card rounded-xl border border-border p-6 flex items-start gap-4">
        ...
        <div>
          ...
          <div className="flex gap-2 mt-2">
            ...
            <span className={`px-2 py-1 text-xs font-semibold rounded-full border ${origin === "Local" ? "text-green bg-green/20 border-green/30" : "text-blue bg-blue/20 border-blue/30"}`}>
              {origin}
            </span>
          </div>
        </div>
      ...
      <div className="mt-8">
        <h2 className="text-xl font-bold text-foreground mb-4">Portfolio Holdings</h2>
        {error ? (
          <div className="p-4 bg-red/10 text-red rounded-lg border border-red/30">Failed to load data.</div>
        ) : (
          <ShareTable
            data={data?.data ?? []}
            total={data?.total ?? 0}
